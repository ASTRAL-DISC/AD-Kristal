local function shootHearts(self, battler, timer, mark)
	local heart = Sprite("enemies/ch3/shadowguy/shoothearts/shoot_heart", battler.x + 42, battler.y - 46)
...
return {
	x_knocksocks = function(cutscene, battler, self)
		...
		local function getSpriteAndOffset(id)
			local selected_sprite = "enemies/ch3/shadowguy/shoothearts/"..id
		...
		local mark = Sprite("enemies/ch3/shadowguy/shoothearts/target", 0, 240)
		mark:setOrigin(0.5)
		mark:setScale(4)
		mark.alpha = 0
		...
		timer:script(function(wait)
			...
			for k,enemy in ipairs(enemies) do
				...
				if socks.hit > count then
					...
					local socks = Sprite("enemies/ch3/shadowguy/socks", enemy.x + enemy.width, enemy.y)
					socks:setScale(2)
