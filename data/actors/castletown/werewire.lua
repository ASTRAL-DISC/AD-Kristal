...
function actor:init()
    ...
    self.path = "npcs/castletown/werewire"
...
function actor:onSpriteInit(sprite)
    if Game:hasRecruit("werewire") then
        sprite.has_wire = false
    else
        sprite.has_wire = true
    end
...
function actor:onSpriteUpdate(sprite)
    ...
    if sprite.has_wire and (wire and wire.update) then
...
function actor:onSpriteDraw(sprite)
    ...
    if sprite.has_wire then
        if wire then
            local offset = sprite:getOffset()

            local wire_curve = Assets.getTexture("npcs/castletown/werewire/wire_curve")
            local wire_base = Assets.getTexture("npcs/castletown/werewire/wire_base")
